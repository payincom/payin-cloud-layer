...
const organization = await request('/api/v1/organizations/org-cloud-layer-sandbox', { headers: authHeaders });
expect(organization?.organization?.id === 'org-cloud-layer-sandbox', 'organization route returns current organization', organization);
const members = await request('/api/v1/organizations/org-cloud-layer-sandbox/members', { headers: authHeaders });
expect(members?.members?.some((member) => member.userId === 'user-runtime-admin'), 'organization members include runtime admin', members);
...
const checkoutOrder = await request(`/api/payment-links/${slug}/orders`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ buyer_email: `buyer+${suffix}@example.com`, chain_id: 'ethereum-sepolia', order_reference: `checkout-order-${suffix}`, redirect_url: `https://merchant.example/thanks/${suffix}` }) });
expect(checkoutOrder?.data?.orderReference === `checkout-order-${suffix}`, 'public payment link order created', checkoutOrder);
expect(checkoutOrder?.data?.redirect_url === `https://merchant.example/thanks/${suffix}`, 'public payment link order preserves redirect_url alias', checkoutOrder);
